...
describe("Points-based Scoring Automation Backend", () => {
  describe("Resolver logic (scoring.ts)", () => {
    ...
    test("resolvePoints handles fallback constraints perfectly", () => {
      ...
      // DSQ, DNF, and DNS always resolve to 0 regardless of position
      ...
      expect(resolvePoints({ scoringRulesMode: "STANDARD", grade: "GI", position: 1, resultStatus: "DNS" })).toBe(0);
      ...
      expect(resolvePoints({ scoringRulesMode: "STANDARD", grade: "GI", position: null, resultStatus: "DNS" })).toBe(0);
    });
  });
