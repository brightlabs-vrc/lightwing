...
function AdminEventDetailPage() {
  ...
  async function handleSelectRace(race: eventmanager.RaceEventDetail) {
    ...
    try {
      ...
      const initialEdits: Record<string, EditedResult> = {}
  ...
  return (
    ...
    >
      ...
      {loadingEventDetail ? (
        <div className="slds-box bg-white slds-align_absolute-center" style={{ background: '#ffffff', borderRadius: '4px', border: '1px solid #dddbda', minHeight: '400px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          <p className="slds-text-heading_small">Loading details for {selectedEvent?.name ?? eventId}...</p>
        </div>
      ) : selectedEvent ? (
