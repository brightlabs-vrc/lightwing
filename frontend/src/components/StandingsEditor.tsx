      ...
      >
        <header className="slds-media slds-media_center slds-has-flexi-truncate">
          <div className="slds-media__figure" style={{ marginRight: '0.5rem' }}>
            <SldsIcon category="utility" name="target" size={18} />
          </div>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {isRaceOngoing && (
            ...
            >
              <SldsIcon category="utility" name="stop" size={14} buttonIcon /> Conclude Race
            </button>
          )}
          {isRaceNotStarted && (
            <button
            ...
            >
              <SldsIcon category="utility" name="play" size={14} buttonIcon /> Start Race
            </button>
          )}
          {!isRaceNotStarted && !isRaceOngoing && (
            ...
            >
              <SldsIcon category="utility" name="clock" size={14} buttonIcon /> Infer Times
            </button>
          )}
          <button
            type="button"
          ...
          {!isRaceNotStarted && (
            ...
            >
              {savingBatch ? 'Saving...' : <><SldsIcon category="utility" name="save" size={14} buttonIcon /> Save ({changeSummary.totalCount})</>}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="slds-card__body" style={{ padding: '16px' }}>
        {isRaceNotStarted ? (
          <div className="slds-align_absolute-center slds-p-around_large text-slate-500" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '200px', textAlign: 'center' }}>
            <SldsIcon category="standard" name="record" size={48} />
            ...
            >
              <SldsIcon category="utility" name="play" size={14} buttonIcon /> Start Race Now
            </button>
          </div>
        ) : (
          <>
              ...
              <div className="slds-m-bottom_medium">
                <AlertBanner
                  ...
                  action={
                    ...
                    >
                      <SldsIcon category="utility" name="stop" size={12} buttonIcon /> Conclude Race
                    </button>
                  }
                >
                  <span style={{ fontWeight: 'bold', fontSize: '12px', color: '#7c2d12' }}>
                    <SldsIcon category="utility" name="record" size={12} color="warning" /> Race is currently Ongoing (Live). You can draft and stage competitor results below, but they can only be saved once the race has concluded.
                  </span>
                </AlertBanner>
              </div>
            )}
  ...
  return (
    <div>
                ...
                <td>
                  <div className="slds-grid" style={{ display: 'flex', gap: '6px' }}>
                    {isDeleted ? (
                      ...
                      >
                        <SldsIcon category="utility" name="undo" size={12} buttonIcon /> Restore
                      </button>
                    ) : isModified || isNew ? (
                      <>
                        <button
                          ...
                          style={{ padding: '2px 8px', fontSize: '11px', flexGrow: 1 }}
                        >
                          <SldsIcon category="utility" name="undo" size={12} buttonIcon /> Reset
                        </button>
                        {savedResult && (
                          <button
                            type="button"
                          ...
                          >
                            <SldsIcon category="utility" name="delete" size={12} buttonIcon /> Remove
                          </button>
                        )}
                      </>
                    ) : savedResult ? (
      ...
      <div className="slds-m-top_medium slds-box" style={{ background: '#f8fafc', border: '1px solid #dddbda', borderRadius: '4px', padding: '12px' }}>
        <h4 className="font-bold text-slate-800" style={{ fontWeight: 'bold' }}><SldsIcon category="utility" name="info" size={16} color="default" /> Explanation of Standings update actions</h4>
