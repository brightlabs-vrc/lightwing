...
function StandingsTable({
  ...
  const getPreviewPoints = (positionStr: string, resultStatus: string): number => {
    // DSQ/DNF/DNS/DEFERRED always resolve to 0 points
              ...
              >
                ...
                <td>
                  ...
                  >
                    ...
                    <option value="DEFERRED">Deferred — Already won an OP</option>
                  </select>
                </td>
                <td>
                  {isDeleted ? (
                    ...
                    <span className="slds-badge slds-theme_success" style={{ padding: '2px 8px', background: savedResult.resultStatus === 'DEFERRED' ? '#64748b' : '#2e7d32', color: '#fff', borderRadius: '4px' }}>
                      {savedResult.resultStatus === 'DSQ' ? 'DSQ' : savedResult.resultStatus === 'DNF' ? 'DNF' : savedResult.resultStatus === 'DNS' ? 'DNS' : savedResult.resultStatus === 'DEFERRED' ? 'Deferred — Already won an OP' : isRaceNotStarted ? `Draw Assigned (${savedResult.gateNumber ?? 'n/a'})` : `Saved (Pos: ${savedResult.position ?? 'n/a'}, Pts: ${savedResult.points})`}
                    </span>
                  ) : (
